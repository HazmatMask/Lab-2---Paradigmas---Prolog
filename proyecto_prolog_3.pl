...
% TDA: SCENE
% FORMATO DEL TDA: [ID,HEIGHT,WIDTH,[QALLY,[ALLY_LIST]],[QENEMY,[ENEMY_LIST]],[Q_OBSTACLE,[OBSTACLE_LIST]]

% CONSTRUCTOR:

newScene(I,H,W,[Q_ALLY|ALLY_LIST],[Q_ENEMY|ENEMY_LIST],[I,H,W,[Q_ALLY|ALLY_LIST],[Q_ENEMY|ENEMY_LIST]]).

% PERTENENCIA:

isScene(S_OUT):-is_list(S_OUT).%,length(S_OUT,5), [I,H,W,A,E] = S_OUT, (number(I),I > 0),(number(H),H > 0),(number(W),W > 0),(number(A),A > 0),(number(E),E > 0).
...
createScene(H,W,Q_ENEMY,D,SEED,S_OUT):-(Q_ENEMY < W/4),useSEED(SEED,6,V_OUT),newScene(V_OUT,H,W,[12],[Q_ENEMY],[12],S_OUT).
