...
createScene(H,W,E,D,_,S_OUT):-      (number(E); number(E),E < W/2),number(D),A is 5-D,((D < 2,newScene(_,H,W,[4|_],[E|_],S_OUT));(D > 3,newScene(_,H,W,[1|_],[E|_],S_OUT));(newScene(_,H,W,[A|_],[E|_],S_OUT))),!.

createScene(H,W,E,_,_,S_OUT):-      (number(H);(number(E),E < W/2);number(W)),newScene(_,H,W,_,[E|_],S_OUT),!.
